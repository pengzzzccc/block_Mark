 ...
 */
export function executeBuy(game, playerId, buyData) {
 ...
 */
export function executeSwap(game, playerId, swapData) {
    const playerIndex = game.players.findIndex((p) => p.id === playerId);
    if (playerIndex !== game.currentPlayerIndex) {
        return { game, success: false, message: '不是你的回合' };
    }
 ...
 */
export function initiateNegotiation(game, fromPlayerId, data) {
 ...
 */
export function handleNegotiationReply(game, playerId, accept) {
    if (!game.negotiation) {
 ...
 */
export function initiateForcedTrade(game, playerId, data) {
    const playerIndex = game.players.findIndex((p) => p.id === playerId);
    if (playerIndex !== game.currentPlayerIndex) {
        return { game, success: false, message: '不是你的回合' };
    }
 ...
 */
export function rollDuelAction(game) {
    if (!game.duelState || game.duelState.phase !== 'rolling')
        return null;
 ...
 */
export function resolveDuel(game) {
 ...
 */
export function advanceToNextPlayer(game) {
