 ...
 */
function searchWinningCombination(blocks) {
...
function generateAllPossiblePairs(nonWilds, wildCount) {
    const pairs = [];
    const seen = new Set();
    ...
    if (wildCount >= 1) {
        for (const b of nonWilds) {
            ...
            const key = blockKey(b);
            if (seen.has(key))
                continue; // already handled as natural pair
...
function pairToDetectedPair(pair) {
...
// ---------------------------------------------------------------
function findTwoSets(nonWilds, wilds) {
...
// Rainbow detection
// ---------------------------------------------------------------
function tryRainbow(nonWilds, wilds) {
...
function generateAllPossibleSets(nonWildOrdinals, wildsAvailable) {
    const candidates = [];
    ...
    const hasBlock = (color, number) => countMap.get(`${color}:${number}`) || 0;
...
function setCandidateToDetectedSet(c) {
    const blocks = [];
...
function removeSetFromBlocks(nonWilds, wilds, set) {
    const remainingNonWilds = [...nonWilds];
    let remainingWilds = [...wilds];
    if (set.wildCount > remainingWilds.length)
        return null;
    ...
    if (set.type === 'triplet') {
        ...
        const numbers = [set.number, set.number + 1, set.number + 2];
