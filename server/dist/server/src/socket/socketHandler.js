import { createRoom, joinRoom, leaveRoom, startGame, getRoom, handleDisconnect, sanitizedGameState, } from './roomManager.js';
import { startTurn, executeBuy, executeSwap, initiateNegotiation, handleNegotiationReply, initiateForcedTrade, rollDuelAction, resolveDuel, advanceToNextPlayer, } from '../game/turnEngine.js';
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('turn:negotiateReply', (data) => {
            ...
            if (offer) {
                ...
                if (fromSocket)
                    fromSocket.emit('negotiate:response', response);
...
function broadcastGameUpdate(io, roomId, gameState) {
 ...
 */
function handlePostAction(io, roomId, gameState) {
